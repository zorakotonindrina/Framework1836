...
public class Utilitaire{
    ...
    {
        ArrayList<String> nomclasses = this.get_AllClass("/WEB-INF/classes/etu1836/framework/modele");
     ...
     public Mapping  geMap(HashMap<String,Mapping> mappingUrls,String cle) throws Exception
    {
     ...
     public ModeleView get_View(HashMap<Class,Object> classSingleton,String cle, HashMap<String,Mapping> mappingUrls, HttpServletRequest request) throws Exception
     {
        ...
        if(isSingleton(classSingleton, myClass) == true){
            obj = classSingleton.get(myClass);
            resetDefaultValueFieldOfObject(obj);
     ...
     public HashMap<Class,Object> getSingleton(HashMap<Class,Object> classSingleton,ServletContext context) throws Exception
     {
        String path = context.getRealPath( "/WEB-INF/classes/etu1836/framework/modele");
        ArrayList <String> classename= this.get_AllClass(path);
        for (int i = 0; i < classename.size(); i++) {
            Class<?> myClass=Class.forName(classename.get(i));
            if(myClass.getAnnotation(Scope.class)!= null){
                String valeure = myClass.getAnnotation(Scope.class).value();
                if(valeure.compareToIgnoreCase("singleton") ==0 ){
                    Object obj= new Object();
                    obj = myClass.newInstance();
                    classSingleton.put(myClass, obj);
                }
     ...
     public Boolean isSingleton(HashMap<Class,Object> classSingleton, Class classe){
        for( Class key : classSingleton.keySet()){
            if(key ==classe){
              return true;
            }
        }
