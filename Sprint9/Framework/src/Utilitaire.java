...
import etu1836.framework.servlet.UploadFile;
...
public class Utilitaire{
     ...
     public Mapping  geMap(HashMap<String,Mapping> mappingUrls,String cle) throws Exception
    {
     ...
     public ModeleView get_View(String cle, HashMap<String,Mapping> mappingUrls, HttpServletRequest request) throws Exception
     {
        ...
        Field[] les_attributs = myClass.getDeclaredFields();
            ...
            String valeur = "";
                for (int i = 0; i < param.length; i++) {
                    ...
                    if((request.getParameter(param[i].getName()) != null) || (request.getParameter(param[i].getName()) != "")){
                        ...
                        String reqContent = request.getContentType();
                        if(reqContent != null && reqContent.startsWith("multipart")){
                            parametre[0] = les_attributs[i].getType();
                            System.out.println("Tafiditra ato @ le upload");
                            Part toupload = request.getPart(les_attributs[i].getName());
                            UploadFile fupld = new UploadFile();
                            fupld.setNom(toupload.getSubmittedFileName());
                            fupld.setData(toupload.getInputStream().readAllBytes());
                     
                            Method theMethod = obj.getClass().getDeclaredMethod("set"+les_attributs[i].getName(),parametre);
                            theMethod.invoke(obj,fupld);
                        }
                        ...
                        System.out.println("Misy file Upload ao \n");
                    }
                }
